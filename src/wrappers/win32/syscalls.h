 ...
 */
#ifndef BE_KERNEL_SYSCALLS_H
#define BE_KERNEL_SYSCALLS_H
...
extern status_t		_kern_generic_syscall(const char *subsystem, uint32_t function, void *buffer, size_t bufferSize);
...
extern status_t		_kern_switch_sem_etc(sem_id releaseSem, sem_id id, uint32_t count, uint32_t flags, bigtime_t timeout);
...
extern status_t		_kern_get_next_sem_info(team_id team, uint32_t *cookie, struct sem_info *info, size_t size);
extern status_t		_kern_set_sem_owner(sem_id id, team_id proc);

/* team & thread syscalls */

// extern thread_id	_kern_load_image(int32_t argCount, const char **args,int32_t envCount, const char **envp, int32_t priority,uint32_t flags);
...
// extern thread_id	_kern_wait_for_child(thread_id child, uint32_t flags,int32_t *_reason, status_t *_returnCode);
// extern status_t	_kern_exec(const char *path, int32_t argc, char * const *argv, int32_t envCount,char * const *environment);
...
// extern thread_id	_kern_spawn_thread(int32_t (*func)(thread_func, void *),const char *name, int32_t priority, void *data1, void *data2);
...
// VFS functions
// extern status_t	_kern_mount(const char *path, const char *device,const char *fs_name, uint32_t flags, const char *args);
...
extern status_t		_kern_entry_ref_to_path(dev_t device, ino_t inode,const char *leaf, char *userPath, size_t pathLength);
...
// extern ssize_t	_kern_select(int numfds, fd_set *readSet, fd_set *writeSet, fd_set *errorSet,bigtime_t timeout, const sigset_t *sigMask);
...
// extern int		_kern_open_query(dev_t device, const char *query, size_t queryLength,uint32_t flags, port_id port, int32_t token);
...
extern status_t		_kern_read_stat(int fd, const char *path, bool traverseLink,struct stat *stat, size_t statSize);
extern status_t		_kern_write_stat(int fd, const char *path,bool traverseLink, const struct stat *stat, size_t statSize, int statMask);
...
// extern status_t	_kern_start_watching(dev_t device, ino_t node, uint32_t flags,port_id port, uint32_t token);
// extern status_t	_kern_stop_watching(dev_t device, ino_t node, uint32_t flags, port_id port, uint32_t token);
...
// area functions
// extern area_id	_kern_create_area(const char *name, void **address, uint32_t addressSpec,	size_t size, uint32_t lock, uint32_t protection);
...
// extern area_id	_kern_transfer_area(area_id area, void **_address, uint32_t addressSpec,team_id target);
// extern status_t	_kern_set_area_protection(area_id area, uint32_t newProtection);
// extern area_id	_kern_clone_area(const char *name, void **_address, uint32_t addressSpec,uint32_t protection, area_id sourceArea);
// extern status_t	_kern_init_heap_address_range(addr_t base, addr_t size);
//
// area_id sys_vm_map_file(const char *name, void **address, int addr_type,addr_t size, int lock, int mapping, const char *path, off_t offset);
...
// extern ssize_t	_kern_read_port_etc(port_id port, int32_t *msgCode, void *msgBuffer,size_t bufferSize, uint32_t flags, bigtime_t timeout);
// extern status_t	_kern_set_port_owner(port_id port, team_id team);
// extern status_t	_kern_write_port_etc(port_id port, int32_t msgCode, const void *msgBuffer,size_t bufferSize, uint32_t flags, bigtime_t timeout);
// extern status_t	_kern_writev_port_etc(port_id id, int32_t msgCode, const iovec *msgVecs,size_t vecCount, size_t bufferSize, uint32_t flags, bigtime_t timeout);
...
// extern status_t	_kern_frame_buffer_update(addr_t baseAddress, int32_t width, int32_t height, int32_t depth, int32_t bytesPerRow);

/* messaging service */
// extern area_id	_kern_register_messaging_service(sem_id lockingSem,sem_id counterSem);
...
extern status_t		_kern_get_disk_device_data(partition_id deviceID, bool deviceOnly,bool shadow, struct user_disk_device_data *buffer,size_t bufferSize, size_t *neededSize);
extern partition_id	_kern_register_file_device(const char *filename);
extern status_t		_kern_unregister_file_device(partition_id deviceID,const char *filename);
    // Only a valid deviceID or filename need to be passed. The other one
    // is -1/NULL. If both is given only filename is ignored.

// disk systems
extern status_t		_kern_get_disk_system_info(disk_system_id id,struct user_disk_system_info *info);
extern status_t		_kern_get_next_disk_system_info(int32_t *cookie,struct user_disk_system_info *info);
extern status_t		_kern_find_disk_system(const char *name,struct user_disk_system_info *info);
extern bool			_kern_supports_defragmenting_partition(partition_id partitionID,int32_t changeCounter, bool *whileMounted);
extern bool			_kern_supports_repairing_partition(partition_id partitionID,int32_t changeCounter, bool checkOnly, bool *whileMounted);
extern bool			_kern_supports_resizing_partition(partition_id partitionID,	int32_t changeCounter, bool *canResizeContents, bool *whileMounted);
extern bool			_kern_supports_moving_partition(partition_id partitionID,int32_t changeCounter, partition_id *unmovable,partition_id *needUnmounting, size_t bufferSize);
extern bool			_kern_supports_setting_partition_name(partition_id partitionID,	int32_t changeCounter);
extern bool			_kern_supports_setting_partition_content_name(partition_id partitionID,	int32_t changeCounter, bool *whileMounted);
extern bool			_kern_supports_setting_partition_type(partition_id partitionID,int32_t changeCounter);
extern bool			_kern_supports_setting_partition_parameters(partition_id partitionID,int32_t changeCounter);
extern bool			_kern_supports_setting_partition_content_parameters(partition_id partitionID, int32_t changeCounter, bool *whileMounted);
extern bool			_kern_supports_initializing_partition(partition_id partitionID,	int32_t changeCounter, const char *diskSystemName);
extern bool			_kern_supports_creating_child_partition(partition_id partitionID,int32_t changeCounter);
extern bool			_kern_supports_deleting_child_partition(partition_id partitionID,int32_t changeCounter);
extern bool			_kern_is_sub_disk_system_for(disk_system_id diskSystemID,partition_id partitionID, int32_t changeCounter);

extern status_t		_kern_validate_resize_partition(partition_id partitionID,int32_t changeCounter, off_t *size);
extern status_t		_kern_validate_move_partition(partition_id partitionID,	int32_t changeCounter, off_t *newOffset);
extern status_t		_kern_validate_set_partition_name(partition_id partitionID,	int32_t changeCounter, char *name);
extern status_t		_kern_validate_set_partition_content_name(partition_id partitionID,	int32_t changeCounter, char *name);
extern status_t		_kern_validate_set_partition_type(partition_id partitionID,	int32_t changeCounter, const char *type);
extern status_t		_kern_validate_initialize_partition(partition_id partitionID,int32_t changeCounter, const char *diskSystemName,char *name, const char *parameters, size_t parametersSize);
extern status_t		_kern_validate_create_child_partition(partition_id partitionID,int32_t changeCounter, off_t *offset, off_t *size, const char *type, const char *parameters,size_t parametersSize);
extern status_t		_kern_get_partitionable_spaces(partition_id partitionID,int32_t changeCounter, struct partitionable_space_data *buffer, int32_t count,int32_t *actualCount);
extern status_t		_kern_get_next_supported_partition_type(partition_id partitionID,int32_t changeCounter, int32_t *cookie, char *type);
extern status_t		_kern_get_partition_type_for_content_type(disk_system_id diskSystemID,const char *contentType, char *type);

// disk device modification
extern status_t		_kern_prepare_disk_device_modifications(partition_id deviceID);
extern status_t		_kern_commit_disk_device_modifications(partition_id deviceID, port_id port, int32_t token, bool completeProgress);
...
extern status_t		_kern_defragment_partition(partition_id partitionID,int32_t changeCounter);
extern status_t		_kern_repair_partition(partition_id partitionID, int32_t changeCounter,bool checkOnly);
extern status_t		_kern_resize_partition(partition_id partitionID, int32_t changeCounter,off_t size);
extern status_t		_kern_move_partition(partition_id partitionID, int32_t changeCounter,off_t newOffset);
extern status_t		_kern_set_partition_name(partition_id partitionID, int32_t changeCounter, const char *name);
extern status_t		_kern_set_partition_content_name(partition_id partitionID,int32_t changeCounter, const char *name);
extern status_t		_kern_set_partition_type(partition_id partitionID, int32_t changeCounter, const char *type);
extern status_t		_kern_set_partition_parameters(partition_id partitionID, int32_t changeCounter, const char *parameters,size_t parametersSize);

extern status_t		_kern_set_partition_content_parameters(partition_id partitionID, int32_t changeCounter, const char *parameters,size_t parametersSize);
extern status_t		_kern_initialize_partition(partition_id partitionID,int32_t changeCounter, const char *diskSystemName,const char *name, const char *parameters,size_t parametersSize);
extern status_t		_kern_uninitialize_partition(partition_id partitionID, int32_t changeCounter);
extern status_t		_kern_create_child_partition(partition_id partitionID, int32_t changeCounter, off_t offset, off_t size, const char *type,const char *parameters, size_t parametersSize, partition_id *childID);
extern status_t		_kern_delete_partition(partition_id partitionID, int32_t changeCounter);
extern status_t		_kern_delete_child_partition(partition_id partitionID, int32_t* changeCounter, partition_id childID,int32_t childChangeCounter);

// jobs
extern status_t		_kern_get_next_disk_device_job_info(int32_t *cookie,struct user_disk_device_job_info *info);
extern status_t		_kern_get_disk_device_job_info(disk_job_id id, struct user_disk_device_job_info *info);
extern status_t		_kern_get_disk_device_job_progress_info(disk_job_id id, struct disk_device_job_progress_info *info);
...
status_t start_disk_device_job_watching(disk_job_id job, port_id, int32_t token,uint32_t flags);
