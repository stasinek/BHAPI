...
//	int nr_free_files;	/* read only */ we don't need this --sadyc
//	int max_files;		/* tunable */ we don't need this --sadyc
};
...
{
	inode->i_blocks += (unsigned long) bytes >> 9;
	bytes &= 511;
	inode->i_bytes += (unsigned short) bytes;
...
#define DECLARE_FSTYPE(var,type,read,flags) \
struct file_system_type var = { type, flags, read, } 
...
{
/*	if (inode->i_flock)
		return __get_lease(inode, mode);*/ // not supported, --sadyc
	return 0;
}
...
extern struct file *filp_open(struct dentry *, char *, int, int);
...
extern int open_namei(char *, int, int, struct nameidata *);
...
extern int FASTCALL __user_walk(char *, unsigned, struct nameidata *);
extern int path_init(char *, unsigned, struct nameidata *);
extern int path_walk(char *, struct nameidata *);
extern int path_lookup(char *, unsigned, struct nameidata *);
extern int link_path_walk(char *, struct nameidata *);
 ...
 */
// we don't need this, yet (used by double_lock) --sadyc
/*
