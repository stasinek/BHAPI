...
void __mark_inode_dirty(struct inode *inode, int flags)
{
	...
	//
	//	--- danny --- nu e initalizat sb->s_dirty nu pun in buffer pentru scris mai tarziu
	//	scriu direct aici :)
	//
	//  --- avand in vedere ca suntem read-only-momentan - nu mai scriu nimic :)
	//
...
void prune_icache(int goal)
{
	 ...
	 */
	if (goal > 0)
		try_to_sync_unused_inodes(NULL);
...
void inode_entry()
{
	KeInitializeSpinLock(&(inode_lock.lock));
}
