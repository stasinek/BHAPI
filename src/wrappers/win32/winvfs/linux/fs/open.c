 ...
 * used by symlinks.
 */
struct file *filp_open(struct dentry * root, char * filename, int flags, int mode)
{
...
struct file *dentry_open(struct dentry *dentry, struct vfsmount *mnt, int flags)
{
	...
	//file_move(f, &inode->i_sb->s_files);	-- danny
 ...
 */
asmlinkage long sys_close(struct file * filp)
{
	//struct file * filp;
	struct files_struct *files = current->files;
