...
/* Windows port and additional code Copyright (C) 2004 Adrian Stanciu <adrian@sadyc.net>
 */
...
// spinlock_t dcache_lock __cacheline_aligned_in_smp = SPIN_LOCK_UNLOCKED;
 spinlock_t dcache_lock __cacheline_aligned_in_smp;
...
//		printk("dentry_iput : %d \n", inode->i_ino);

		dentry->d_inode = NULL;
		list_del_init(&dentry->d_alias);

		// --- danny --- super bug ---
		// 00000035, kernel is releasing a spinlock when not at DISPATCH_LEVEL.
		// 00000000, current IRQL,
		// watch -> dcache_lock->irql = 2 -> incearca sa ridice la dispatch level irql-ul
		// --- danny --- temporary fix : disable spinlocks -> /linux/spinlock.h
...
void dput(struct dentry *dentry)
{
	...
	printk("--- dput: %s count:%d\n", dentry->d_iname, dentry->d_count);
//	if (dentry->d_inode)
//		printk("*** dput2: %s %d count:%d\n", dentry->d_iname, dentry->d_inode->i_ino, dentry->d_count);
...
{
	printk("dcache: prune_dcache(%d) stat.nr_dentry= %d stat.nr_unused= %d\n", count, dentry_stat.nr_dentry, dentry_stat.nr_unused);
	spin_lock(&dcache_lock);
	for (;;) {
	...
	spin_unlock(&dcache_lock);
	printk("dcache: prune_dcache finished: stat.nr_dentry= %d stat.nr_unused= %d\n", count, dentry_stat.nr_dentry, dentry_stat.nr_unused);
}
...
void shrink_dcache_parent(struct dentry * parent)
{
	...
	while ((found = select_parent(parent)) != 0) {
		// tofix: this is a hack --sadyc
		// if we free the last dentry the memory pool gets corrupted
		if (found == 1) break; 
		prune_dcache(found);
	}
}
...
int shrink_dcache_memory(int priority, unsigned int gfp_mask)
{
	...
	// tofix: this is a hack --sadyc
	// we can't free the last dentry because the memory pool gets corrupted
	if (count>1) prune_dcache(count-1);
...
struct dentry * d_alloc(struct dentry * parent, const struct qstr *name)
{
	...
	ASSERT(dentry->d_count.counter == 1);
...
struct dentry * d_alloc_root(struct inode * root_inode)
{
	...
	const struct qstr temp_qstr = { "/", 1, 0 };

	if (root_inode) {
		res = d_alloc(NULL, &temp_qstr);
		if (res) {
...
int d_validate(struct dentry *dentry, struct dentry *dparent)
{
	...
	//	goto out;
// skip this, because i don't know to se high_memory --sadyc
...
//	const unsigned char *old_name, *new_name;
	unsigned char *old_name, *new_name;  // visual studio e nemultumit de const, --sadyc
...
void d_move(struct dentry * dentry, struct dentry * target)
{
	...
	do_switch_ptr(dentry->d_parent, target->d_parent);
	...
	*retval = '/';
	...
	for (;;) {
		...
		if (dentry == vfsmnt->mnt_root || IS_ROOT(dentry)) {
			/* Global root? */
			BUG();  // this should not happen  --sadyc
...
//global_root:   because Visual C complains --sadyc
	namelen = dentry->d_name.len;
	buflen -= namelen;
	if (buflen >= 0) {
...
//		init_waitqueue_head(&bh->b_wait); --commented by me, --sadyc
	}
}
...
void dcache_unload()
{
	shrink_dcache_memory(1, __GFP_FS);
	kfree(dentry_hashtable);
