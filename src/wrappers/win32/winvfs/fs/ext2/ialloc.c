...
void ext2_free_inode (struct inode * inode)
{
	...
	if (ino < (unsigned) EXT2_FIRST_INO(sb) ||
...
repeat:
	...
	if (i >= (signed) EXT2_INODES_PER_GROUP(sb))
		goto bad_count;
	...
	if (ino < (ino_t) EXT2_FIRST_INO(sb) || ino > le32_to_cpu(es->s_inodes_count)) {
...
//	inode->i_generation = event++;  //tofix --sadyc
