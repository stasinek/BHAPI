			       ...
			       unsigned long bitmap_nr)
{
	...
	if (!gdp)
		goto error_out;
	retval = 0;
	printk("ext2: read_block_bitmap block_group= %u bitmap_nr= %l\n", block_group, bitmap_nr);
...
repeat:
	...
	if (le16_to_cpu(gdp->bg_free_blocks_count) > 0) {
		...
		if (k < (signed) EXT2_BLOCKS_PER_GROUP(sb)) {
			j = k;
			goto search_back;
		}
	 ...
	 */
	for (k = 0; k < (signed) sb->u.ext2_sb.s_groups_count; k++) {
		i++;
		if (i >= (signed) sb->u.ext2_sb.s_groups_count)
			i = 0;
		gdp = ext2_get_group_desc (sb, i, &bh2);
		if (!gdp)
			goto io_error;
	...
	if (k >= (signed) sb->u.ext2_sb.s_groups_count)
		goto out;
	bitmap_nr = load_block_bitmap (sb, i);
	if (bitmap_nr < 0)
		goto io_error;
	...
	j = (r - bh->b_data) << 3;
	if (j < (signed) EXT2_BLOCKS_PER_GROUP(sb))
		goto search_back;
	else
...
got_block:
	...
	if (tmp == le32_to_cpu(gdp->bg_block_bitmap) ||
	    ...
	    in_range (tmp, (signed) le32_to_cpu(gdp->bg_inode_table),
		       (signed) EXT2_SB(sb)->s_itb_per_group)) {
		ext2_error (sb, "ext2_new_block",
	...
	if (prealloc_count && !*prealloc_count) {
		...
		*prealloc_block = next_block;
		/* Writer: end */
		for (k = 1;
		     k < prealloc_goal && (j + k) < (signed) EXT2_BLOCKS_PER_GROUP(sb);
		     k++, next_block++) {
	...
	if (j >= (signed) le32_to_cpu(es->s_blocks_count)) {
		ext2_error (sb, "ext2_new_block",
