...
void ext2_discard_prealloc (struct inode * inode)
{
	...
	if (inode->u.ext2_i.i_prealloc_count) {
		unsigned short total = (unsigned short) inode->u.ext2_i.i_prealloc_count;
...
//	printk("ext2_get_block: inode=%d, iblock=%d depth=%d\n", inode->i_ino, iblock, depth);
...
void ext2_truncate (struct inode * inode)
{
	...
	iblock = (long) (inode->i_size + blocksize-1)
...
void ext2_read_inode (struct inode * inode)
{
	...
	if ((inode->i_ino != EXT2_ROOT_INO && inode->i_ino != EXT2_ACL_IDX_INO &&
	     inode->i_ino != EXT2_ACL_DATA_INO &&
	     inode->i_ino < (ino_t) EXT2_FIRST_INO(inode->i_sb)) ||
...
//	inode->i_version = ++event;  // tofix --sadyc
