		 ...
		 const char * fmt, ...)
{
	...
	va_start (args, fmt);
	RtlStringCbVPrintfA (error_buf, sizeof(error_buf), fmt, args);
	va_end (args);
	if (test_opt (sb, ERRORS_PANIC) ||
	    (le16_to_cpu(sb->u.ext2_sb.s_es->s_errors) == EXT2_ERRORS_PANIC &&
		!test_opt (sb, ERRORS_CONT) && !test_opt (sb, ERRORS_RO))) {
			printk ("EXT2-fs panic (device %s): %s: %s\n",
			  ...
			  unsigned long * mount_options)
{
	...
	for (this_char = strtok1 (options, ",");
		...
		if (!strcmp (this_char, "bsddf"))
			...
			*resgid = (unsigned short) simple_strtoul (value, &value, 0);
			if (*value) {
			...
			*resuid = (unsigned short) simple_strtoul (value, &value, 0);
			if (*value) {
...
static int ext2_check_descriptors (struct super_block * sb)
{
	...
	for (i = 0; i < (signed) sb->u.ext2_sb.s_groups_count; i++)
	{
...
{
	struct ext2_super_block * es;
	unsigned short resuid = (unsigned short) sb->u.ext2_sb.s_resuid;
	unsigned short resgid = (unsigned short) sb->u.ext2_sb.s_resgid;
...
int ext2_statfs (struct super_block * sb, struct statfs * buf)
{
	...
	if (test_opt (sb, MINIX_DF))
		 ...
		 */
		for (i = 0; i < (signed) EXT2_SB(sb)->s_groups_count; i++)
	...
	if (buf->f_bfree < (signed) le32_to_cpu(sb->u.ext2_sb.s_es->s_r_blocks_count))
		buf->f_bavail = 0;
