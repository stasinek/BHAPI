...
static struct dentry *minix_lookup(struct inode * dir, struct dentry *dentry)
{
	...
	if (dentry->d_name.len > (unsigned int)dir->i_sb->u.minix_sb.s_namelen)
...
static int minix_mknod(struct inode * dir, struct dentry *dentry, int mode, int rdev)
{
	...
	if (inode) {
		...
		minix_set_inode(inode, (dev_t)rdev);
		mark_inode_dirty(inode);
...
struct inode_operations minix_dir_inode_operations = {
	minix_create, minix_lookup,	minix_link,	minix_unlink,
	minix_symlink, minix_mkdir,	minix_rmdir,
	minix_mknod, minix_rename, NULL, NULL, NULL, NULL, 
