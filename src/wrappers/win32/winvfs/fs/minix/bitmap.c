...
void minix_free_block(struct inode * inode, int block)
{
	...
	if (block < (int)sb->u.minix_sb.s_firstdatazone ||
	    block >= (int)sb->u.minix_sb.s_nzones) {
...
repeat:
	...
	if (j < (int)sb->u.minix_sb.s_firstdatazone ||
	    j >= (int)sb->u.minix_sb.s_nzones)
		return 0;
	return j;
}
...
struct inode * minix_new_inode(const struct inode * dir, int * error)
{
	...
	*error = -ENOSPC;
	...
	mark_buffer_dirty(bh);
	j += i*8192;
	if (!j || j > (int)inode->i_sb->u.minix_sb.s_ninodes) {
		iput(inode);
		unlock_super(sb);
		return NULL;
	}
