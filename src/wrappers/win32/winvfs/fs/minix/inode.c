...
static void minix_put_super(struct super_block *sb)
{
	...
	for (i = 0; i < (int)sb->u.minix_sb.s_imap_blocks; i++)
		brelse(sb->u.minix_sb.s_imap[i]);
	for (i = 0; i < (int)sb->u.minix_sb.s_zmap_blocks; i++)
				     ...
				     int silent)
{
	...
	block=2;
	for (i=0 ; i < (int)sbi->s_imap_blocks ; i++) {
		if (!(sbi->s_imap[i]=sb_bread(s, block)))
			goto out_no_bitmap;
		block++;
	}
	for (i=0 ; i < (int)sbi->s_zmap_blocks ; i++) {
...
void minix_set_inode(struct inode *inode, dev_t rdev)
{
	if (S_ISREG(inode->i_mode)) {
		...
		init_special_inode(inode, inode->i_mode, (int)rdev);
}
...
static void V1_minix_read_inode(struct inode * inode)
{
	...
	minix_set_inode(inode, (dev_t)raw_inode->i_zone[0]);
	brelse(bh);
}
...
static struct buffer_head * V1_minix_update_inode(struct inode * inode)
{
	...
	raw_inode->i_nlinks = (__u8)inode->i_nlink;
	raw_inode->i_size = (__u32)inode->i_size;
	raw_inode->i_time = inode->i_mtime;
	if (S_ISCHR(inode->i_mode) || S_ISBLK(inode->i_mode))
		raw_inode->i_zone[0] = (__u16)kdev_t_to_nr(inode->i_rdev);
	else for (i = 0; i < 9; i++)
