...
/* Fake . and .. for the root directory. */
	...
	if (inode->i_ino == MSDOS_ROOT_INO) {
		while (cpos < 2) {
			if (filldir(dirent, "..", (int)cpos+1, cpos, MSDOS_ROOT_INO, DT_DIR) < 0)
				return 0;
			cpos++;
			filp->f_pos++;
		}
...
ParseLong:
	...
	lpos = (unsigned long)(cpos - (long_slots+1)*sizeof(struct msdos_dir_entry));
...
#else
	/* tavi:
		inainte sa se apeleze filldir trebuie construit inodul FAT, altfel toate
		informatiile mai putin numarul inodului sunt in aer din cauza
		cache-ului FAT
		
		in Linux nu e nici o problema, pentru ca readdir nu trebuie sa
		intoarca decat numarul inodului si _eventual_ tipul fisierului
		
		in windows readdir trebuie sa intoarca si informatiile de la stat

		bucata de cod de mai jos e luata din fat_lookup
	*/

		} else {
	
			int res;
			loff_t i_pos, sector, offset=cpos; 
			struct inode *inode2;


			printk("about to fat_build_inode\n");

			if ((sector = fat_bmap(inode,(int)(offset >> sb->s_blocksize_bits))) == -1) {
				printk("fat_readdir: fat_bmap failed\n");
				goto FillFailed;
			}

			offset &= sb->s_blocksize - 1;
			i_pos = ((loff_t)sector << sb->s_blocksize_bits) + offset;

			inode2 = fat_build_inode(sb, de, i_pos, &res);
	
			if (res) {
				printk("fat_readdir: fat_build_inode failed\n");
				goto FillFailed;
			}
