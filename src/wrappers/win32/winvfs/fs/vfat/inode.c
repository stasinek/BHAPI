			 ...
			 char *cvf_format, char *cvf_options)
{
	...
	opts->fs_umask = (unsigned short)current->fs->umask;
	...
	*debug = *fat = 0;
	...
	save = 0;
	savep = NULL;
	for (this_char = strtok1(options,","); this_char;
	     this_char = strtok1(NULL,",")) {
		...
		if (!strcmp(this_char,"check") && value) {
			...
			if (!value || !*value) ret = 0;
				...
				opts->fs_umask = (unsigned short)simple_strtoul(value,&value,8);
				if (*value) ret = 0;
			}
		}
			...
			opts->codepage = (unsigned short)simple_strtoul(value,&value,0);
			if (*value) ret = 0;
...
static void fat_read_root(struct inode *inode)
{
	...
	inode->i_blocks = (unsigned long)((inode->i_size + inode->i_blksize - 1)
			   & ~(inode->i_blksize - 1)) >> 9;
	MSDOS_I(inode)->i_logstart = 0;
	MSDOS_I(inode)->mmu_private = (unsigned long)inode->i_size;
 ...
 */
	...
	if (!error) {
		...
		error = !sbi->fats || (sbi->dir_entries & (sbi->dir_per_block - 1))
			|| (int)sbi->clusters + 2 > fat_clusters + MSDOS_MAX_EXTRA
	...
	if (i < 0)
		if ((int)sbi->clusters + 2 > fat_clusters)
	...
	RtlStringCbPrintfA(buf, sizeof(buf), "cp%d", cp);
...
int fat_statfs(struct super_block *sb,struct statfs *buf)
{
	...
	if (MSDOS_SB(sb)->free_clusters != -1)
		...
		free = 0;
		for (nr = 2; nr < (int)MSDOS_SB(sb)->clusters+2; nr++)
...
static void fat_fill_inode(struct inode *inode, struct msdos_dir_entry *de)
{
	...
	if ((de->attr & ATTR_DIR) && !IS_FREE(de->name)) {
		...
		inode->i_nlink = (nlink_t)fat_subdirs(inode);
	...
	printk("fat_fill_inode: mode=0%o\n", inode->i_mode);
}
...
retry:
	...
	lock_kernel();
	if (!(bh = fat_bread(sb, (int)(i_pos >> MSDOS_SB(sb)->dir_per_block_bits)))) {
	...
	if (MSDOS_SB(sb)->options.isvfat) {
		...
		raw_entry->ctime_ms = (__u8)MSDOS_I(inode)->i_ctime_ms;
