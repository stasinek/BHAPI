...
// needed because we use GNU ntifs.h --sadyc
...
// -- danny daca aici return 0 atunci cached_lookup nu va mai lua dentry-ul din cache
// si va face dput si va retuna NULL
// la fel se pare ca face si real_lookup intr-un anumit caz
// si link path walk face asta pentru . si .. - dar noi nu avem nici o data . si ..
// dar vad ca daca incerc sa deschid de ex i:\.\..\.\..\..\.\ va deschide doar \
// deci probabil ca win face parsarea cu . si .. inainte de a trimite-o la driver
// e mai safe sa returnez momentan 1 pentru debug
// TO FIX
...
static int parse_options(char *options,	struct fat_mount_options *opts)
{
	...
	savep = NULL;
	ret = 1;
	for (this_char = strtok1(options,","); this_char; this_char = strtok1(NULL,",")) {
...
static int vfat_valid_longname(const char *name, int len, int xlate)
{
	...
	if (baselen == 3) {
		for (reserved = reserved3_names; *reserved; reserved++) {
			if (!strnicmp1(name,*reserved,baselen))
				return -EINVAL;
		}
	} else if (baselen == 4) {
				 ...
				 char *name_res, unsigned char *lcase)
{
	...
	*lcase = 0;
	...
	Seed = ExAllocatePool(PagedPool, sizeof(PULONG));
	*Seed = CURRENT_TIME;
	jiffies = RtlRandom(Seed);
	ExFreePool(Seed);
	i = jiffies & 0xffff;
	sz = (jiffies >> 16) & 0x7;
	if (baselen>2) {
	...
	while (1) {
		RtlStringCbPrintfA(buf, sizeof(buf), "%04X", i);	
...
shortname:
	...
	de->ctime_ms = (__u8)de->ctime = (__u8)de->time = 0;
	de->start = 0;
	de->starthi = 0;
	de->size = 0;
