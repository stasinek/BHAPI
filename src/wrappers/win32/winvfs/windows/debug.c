...
ULONG VfsGetProcessNameOffset(VOID)
{
	...
	{
		if(!strncmp(SYSTEM_PROCESS_NAME, (PCHAR) Process + i, strlen(SYSTEM_PROCESS_NAME)))
		{
			return i;
		}
	}

	KdPrint(("VFS: *** VfsGetProcessNameOffset failed ***\n"));
...
VOID VfsDbgPrintCall(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
...
#ifdef _GNU_NTIFS_
	PEXTENDED_IO_STACK_LOCATION IrpSpEx;
#else
	PIO_STACK_LOCATION		IrpSpEx;
#endif
	PFILE_OBJECT			FileObject;
	PUCHAR					FileName;
	char					FileNameAllocated = 0;
	struct vfs_fcb			*Fcb;
...
#ifdef _GNU_NTIFS_
	IrpSpEx = (PEXTENDED_IO_STACK_LOCATION) IrpSp;
#else
	IrpSpEx = IrpSP;
#endif
	...
	if (DeviceObject == vfs_data.device)
	{
		FileName = "\\vfs" ;
	}
	else if (FileObject && FileObject->FsContext)
	{
		...
		else if (Fcb->identifier.type == VFS_NODE_TYPE_FCB)
		{
			struct vfs_ccb* ccb = (struct vfs_ccb*)FileObject->FsContext2;
			int len = ccb->file->f_dentry->d_name.len;

			FileName = ExAllocatePoolWithTag(VFSPOOL, len+1, '1GBD');
			RtlCopyMemory(FileName, ccb->file->f_dentry->d_name.name, len);
			FileName[len] = 0;
			FileNameAllocated = 1;
		}
	}
	...
	case IRP_MJ_CREATE:
		...
		if (FileName)
		{
			DbgPrint("VFS: %-16.16s %-31s %s\n", VfsGetCurrentProcessName(),
				IrpMjStrings[IrpSp->MajorFunction], FileName);
		}
		else if (IrpSp->FileObject->FileName.Buffer)
		{
			DbgPrint("VFS: %-16.16s %-31s %S\n", VfsGetCurrentProcessName(),
				IrpMjStrings[IrpSp->MajorFunction], IrpSp->FileObject->FileName.Buffer);
		}
		else
		{
			DbgPrint("VFS: %-16.16s %-31s %s\n", VfsGetCurrentProcessName(),
	...
	case IRP_MJ_READ:
		...
		{
			DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_COMPLETE\n", 
				VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction], FileName);
		}
		else
		{
			DbgPrint("VFS: %-16.16s %-31s %s\n", VfsGetCurrentProcessName(),
				IrpMjStrings[IrpSp->MajorFunction], FileName);
            DbgPrint("VFS: %-16.16s Offset: %I64u Length: %u Key: %u %s%s%s%s%s%s\n",
				VfsGetCurrentProcessName(), IrpSp->Parameters.Read.ByteOffset.QuadPart,
	...
	case IRP_MJ_QUERY_INFORMATION:
		...
		{
			DbgPrint("VFS: %-16.16s %-31s %s %s\n", VfsGetCurrentProcessName(),
				IrpMjStrings[IrpSp->MajorFunction], FileName,
				FileInformationClassStrings[FileInformationClass]);
		}
		else
		{
			DbgPrint("VFS: %-16.16s %-31s %s Unknown FileInformationClass %u\n",
				VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction], FileName,
	...
	case IRP_MJ_SET_INFORMATION:
		...
		{
			DbgPrint("VFS: %-16.16s %-31s %s Unknown FileInformationClass %u\n",
				VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
	...
	case IRP_MJ_QUERY_VOLUME_INFORMATION:
		...
		{
            DbgPrint("VFS: %-16.16s %-31s %s Unknown FsInformationClass %u\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
    ...
    case IRP_MJ_DIRECTORY_CONTROL:
        ...
        {
            FileInformationClass = IrpSpEx->Parameters.QueryDirectory.FileInformationClass;
            ...
            if (FileInformationClass <= FileMaximumInformation)
            {
                ...
                if (IrpSpEx->Parameters.QueryDirectory.FileName)
                {
                    DbgPrint("VFS: %-16.16s FileName: %.*S FileIndex: %#x %s%s%s\n",
                        VfsGetCurrentProcessName(),
                        IrpSpEx->Parameters.QueryDirectory.FileName->Length / 2,
                        IrpSpEx->Parameters.QueryDirectory.FileName->Buffer,
                        IrpSpEx->Parameters.QueryDirectory.FileIndex,
                ...
                {
                    DbgPrint("VFS: %-16.16s FileName: FileIndex: %#x %s%s%s\n",
                        VfsGetCurrentProcessName(),
                        IrpSpEx->Parameters.QueryDirectory.FileIndex,
                        (IrpSp->Flags & SL_RESTART_SCAN ? "SL_RESTART_SCAN " : ""),
                        (IrpSp->Flags & SL_RETURN_SINGLE_ENTRY ? "SL_RETURN_SINGLE_ENTRY " : ""),
            ...
            {
                DbgPrint(
                    "VFS: %-16.16s %-31s %s Unknown FileInformationClass %u\n",
                    VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
                    FileName, FileInformationClass);
            }
        }
        ...
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_NOTIFY_CHANGE_DIRECTORY\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
                FileName);

            DbgPrint("VFS: CompletionFilter: %#x %s\n",
                IrpSpEx->Parameters.NotifyDirectory.CompletionFilter,
                (IrpSp->Flags & SL_WATCH_TREE ? "SL_WATCH_TREE " : "")
                );
        }
        else
        {
            DbgPrint("VFS: %-16.16s %-31s %s Unknown minor function %#x\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
                FileName, IrpSp->MinorFunction);
        }
    ...
    case IRP_MJ_FILE_SYSTEM_CONTROL:
        ...
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_USER_FS_REQUEST FsControlCode: %#x\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
                FileName, IrpSpEx->Parameters.FileSystemControl.FsControlCode);
        }
        else if (IrpSp->MinorFunction == IRP_MN_MOUNT_VOLUME)
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_MOUNT_VOLUME DeviceObject: %#x\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
                FileName, IrpSp->Parameters.MountVolume.DeviceObject);
        }
        else if (IrpSp->MinorFunction == IRP_MN_VERIFY_VOLUME)
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_VERIFY_VOLUME DeviceObject: %#x\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
                FileName, IrpSp->Parameters.VerifyVolume.DeviceObject);
        }
        else if (IrpSp->MinorFunction == IRP_MN_LOAD_FILE_SYSTEM)
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_LOAD_FILE_SYSTEM\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction], FileName);
        }
        else
        {
    ...
    case IRP_MJ_DEVICE_CONTROL:
        ...
        DbgPrint("VFS: %-16.16s %-31s %s IoControlCode: %#x\n",
            VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction], FileName,
    ...
    case IRP_MJ_LOCK_CONTROL:
        ...
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_LOCK Offset: %I64u Length: %I64u Key: %u %s%s\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
				FileName,
                IrpSpEx->Parameters.LockControl.ByteOffset.QuadPart,
                IrpSpEx->Parameters.LockControl.Length->QuadPart,
                IrpSpEx->Parameters.LockControl.Key,
        ...
        {
            DbgPrint(
                "VFS: %-16.16s %-31s %s IRP_MN_UNLOCK_SINGLE Offset: %I64u Length: %I64u Key: %u\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction], FileName,
                IrpSpEx->Parameters.LockControl.ByteOffset.QuadPart,
                IrpSpEx->Parameters.LockControl.Length->QuadPart,
                IrpSpEx->Parameters.LockControl.Key);
        ...
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_UNLOCK_ALL\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction], FileName);
        }
        else if (IrpSp->MinorFunction & IRP_MN_UNLOCK_ALL_BY_KEY)
        {
            DbgPrint("VFS: %-16.16s %-31s %s IRP_MN_UNLOCK_ALL_BY_KEY Key: %u\n",
                VfsGetCurrentProcessName(), IrpMjStrings[IrpSp->MajorFunction],
                FileName,
                IrpSpEx->Parameters.LockControl.Key
                );
	...
	if (FileNameAllocated) ExFreePool(FileName);
}
...
VOID VfsDbgPrintComplete(IN PIRP Irp)
{
    ...
    //{
        ...
        DbgPrint("VFS: %-16.16s %-31s *** Status: %s (%#x) ***\n",
            VfsGetCurrentProcessName(),
            IrpMjStrings[IrpSp->MajorFunction],
            VfsNtStatusToString(Irp->IoStatus.Status),
            Irp->IoStatus.Status);
    //}
}
