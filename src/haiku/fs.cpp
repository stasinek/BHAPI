...
normalize_dir_path(const char *path, string &normalizedPath)
{
	...
	return normalize_dir_path(path, node_ref(st), normalizedPath);
}
...
typedef map<node_ref, string> DirPathMap;
...
BPrivate::get_path(int fd, const char *name, string &path)
{
	...
	if (fd >= 0 && !(name && name[0] == '/')) {
		...
		return ::get_path((node_ref*)NULL, name, path);
}
...
_kern_open_parent_dir(int fd, char *name, size_t nameLength)
{
	...
	realPath += "/..";
	string entryName;
	error = find_dir_entry(realPath.c_str(), node_ref(st),
		entryName, false);
	if (error != B_OK)
		return error;
