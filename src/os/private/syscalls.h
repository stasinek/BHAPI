...
BHAPI_IMPEXP status_t		_kern_generic_syscall(const char *subsystem, uint32_t function,
...
BHAPI_IMPEXP status_t		_kern_mutex_lock(int32_t* mutex, const char* name,
                        uint32_t flags, bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_mutex_unlock(int32_t* mutex, uint32_t flags);
BHAPI_IMPEXP status_t		_kern_mutex_switch_lock(int32_t* fromMutex, int32_t* toMutex,
                        const char* name, uint32_t flags, bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_mutex_sem_acquire(int32_t* sem, const char* name,
                        uint32_t flags, bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_mutex_sem_release(int32_t* sem);
...
BHAPI_IMPEXP status_t		_kern_switch_sem_etc(sem_id releaseSem, sem_id id,
                        uint32_t count, uint32_t flags, bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_acquire_sem(sem_id id);
BHAPI_IMPEXP status_t		_kern_acquire_sem_etc(sem_id id, uint32_t count, uint32_t flags,
                        bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_release_sem(sem_id id);
BHAPI_IMPEXP status_t		_kern_release_sem_etc(sem_id id, uint32_t count, uint32_t flags);
BHAPI_IMPEXP status_t		_kern_get_sem_count(sem_id id, int32_t* thread_count);
BHAPI_IMPEXP status_t		_kern_get_sem_info(sem_id semaphore, struct sem_info *info,
                        size_t size);
BHAPI_IMPEXP status_t		_kern_get_next_sem_info(team_id team, int32_t *cookie,
...
BHAPI_IMPEXP status_t		_kern_realtime_sem_open(const char* name,
                        int openFlagsOrShared, mode_t mode, uint32_t semCount,
...
BHAPI_IMPEXP thread_id	_kern_load_image(const char* const* flatArgs,
                        size_t flatArgsSize, int32_t argCount, int32_t envCount,
                        int32_t priority, uint32_t flags, port_id errorPort,
                        uint32_t errorToken);
...
BHAPI_IMPEXP pid_t		_kern_wait_for_child(thread_id child, uint32_t flags,
                        siginfo_t* info);
BHAPI_IMPEXP status_t		_kern_exec(const char *path, const char* const* flatArgs,
                        size_t flatArgsSize, int32_t argCount, int32_t envCount,
                        mode_t umask);
BHAPI_IMPEXP thread_id	_kern_fork(void);
BHAPI_IMPEXP pid_t		_kern_process_info(pid_t process, int32_t which);
...
BHAPI_IMPEXP status_t		_kern_send_data(thread_id thread, int32_t code,
                        const void *buffer, size_t bufferSize);
BHAPI_IMPEXP int32_t		_kern_receive_data(thread_id *_sender, void *buffer,
                        size_t bufferSize);
...
BHAPI_IMPEXP status_t		_kern_get_next_thread_info(team_id team, int32_t *cookie,
                        thread_info *info);
BHAPI_IMPEXP status_t		_kern_get_team_info(team_id id, team_info *info);
BHAPI_IMPEXP status_t		_kern_get_next_team_info(int32_t *cookie, team_info *info);
BHAPI_IMPEXP status_t		_kern_get_team_usage_info(team_id team, int32_t who,
                        team_usage_info *info, size_t size);
BHAPI_IMPEXP status_t		_kern_get_extended_team_info(team_id teamID, uint32_t flags,
                        void* buffer, size_t size, size_t* _sizeNeeded);

BHAPI_IMPEXP status_t		_kern_start_watching_system(int32_t object, uint32_t flags,
                        port_id port, int32_t token);
BHAPI_IMPEXP status_t		_kern_stop_watching_system(int32_t object, uint32_t flags,
                        port_id port, int32_t token);

BHAPI_IMPEXP status_t		_kern_block_thread(uint32_t flags, bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_unblock_thread(thread_id thread, status_t status);
BHAPI_IMPEXP status_t		_kern_unblock_threads(thread_id* threads, uint32_t count,
                        status_t status);

BHAPI_IMPEXP bigtime_t	_kern_estimate_max_scheduling_latency(thread_id thread);

BHAPI_IMPEXP status_t		_kern_set_scheduler_mode(int32_t mode);
BHAPI_IMPEXP int32_t		_kern_get_scheduler_mode(void);
...
// signal functions
BHAPI_IMPEXP status_t		_kern_send_signal(int32_t id, uint32_t signal,
...
BHAPI_IMPEXP status_t		_kern_get_image_info(image_id id, image_info *info,
                        size_t size);
BHAPI_IMPEXP status_t		_kern_get_next_image_info(team_id team, int32_t *cookie,
                        image_info *info, size_t size);
BHAPI_IMPEXP status_t		_kern_read_kernel_image_symbols(image_id id,
                        elf_sym* symbolTable, int32_t* _symbolCount,
...
BHAPI_IMPEXP dev_t		_kern_mount(const char *path, const char *device,
                        ...
                        size_t argsLength);
BHAPI_IMPEXP status_t		_kern_unmount(const char *path, uint32_t flags);
...
BHAPI_IMPEXP status_t		_kern_write_fs_info(dev_t device, const struct fs_info *info,
                        int mask);
BHAPI_IMPEXP dev_t		_kern_next_device(int32_t *_cookie);
...
BHAPI_IMPEXP ssize_t		_kern_write_attr(int fd, const char *attribute, uint32_t type,
...
BHAPI_IMPEXP int			_kern_open_query(dev_t device, const char *query,
                        size_t queryLength, uint32_t flags, port_id port,
                        int32_t token);
...
BHAPI_IMPEXP ssize_t		_kern_writev(int fd, off_t pos, const struct iovec *vecs,
                        size_t count);
BHAPI_IMPEXP status_t		_kern_ioctl(int fd, uint32_t cmd, void *data, size_t length);
BHAPI_IMPEXP ssize_t		_kern_read_dir(int fd, struct dirent *buffer,
                        size_t bufferSize, uint32_t maxCount);
...
BHAPI_IMPEXP status_t		_kern_get_next_fd_info(team_id team, uint32_t *_cookie,
...
BHAPI_IMPEXP status_t		_kern_socketpair(int family, int type, int protocol,
                        int *socketVector);
BHAPI_IMPEXP status_t		_kern_get_next_socket_stat(int family, uint32_t *cookie,
                        struct net_stat *stat);

// node monitor functions
BHAPI_IMPEXP status_t		_kern_stop_notifying(port_id port, uint32_t token);
BHAPI_IMPEXP status_t		_kern_start_watching(dev_t device, ino_t node, uint32_t flags,
...
BHAPI_IMPEXP status_t		_kern_set_timezone(int32_t timezoneOffset, const char *name,
                        size_t nameLength);
BHAPI_IMPEXP status_t		_kern_get_timezone(int32_t *_timezoneOffset, char *name,
                        size_t nameLength);
...
BHAPI_IMPEXP status_t		_kern_snooze_etc(bigtime_t time, int timebase, int32_t flags,
                        bigtime_t* _remainingTime);

BHAPI_IMPEXP int32_t		_kern_create_timer(clockid_t clockID, thread_id threadID,
                        uint32_t flags, const struct sigevent* event,
                        const struct thread_creation_attributes*
                            threadAttributes);
BHAPI_IMPEXP status_t		_kern_delete_timer(int32_t timerID, thread_id threadID);
BHAPI_IMPEXP status_t		_kern_get_timer(int32_t timerID, thread_id threadID,
                        struct user_timer_info* info);
BHAPI_IMPEXP status_t		_kern_set_timer(int32_t timerID, thread_id threadID,
...
BHAPI_IMPEXP status_t		_kern_reserve_address_range(addr_t* _address,
                        uint32_t addressSpec, addr_t size);
...
BHAPI_IMPEXP status_t		_kern_get_memory_properties(team_id teamID,
                        const void *address, uint32_t* _protected, uint32_t* _lock);

/* kernel port functions */
BHAPI_IMPEXP port_id		_kern_create_port(int32_t queue_length, const char *name);
...
BHAPI_IMPEXP status_t		_kern_get_next_port_info(team_id team, int32_t *cookie,
                        struct port_info *info);
BHAPI_IMPEXP ssize_t		_kern_port_buffer_size_etc(port_id port, uint32_t flags,
                        bigtime_t timeout);
BHAPI_IMPEXP int32_t		_kern_port_count(port_id port);
BHAPI_IMPEXP ssize_t		_kern_read_port_etc(port_id port, int32_t *msgCode,
                        void *msgBuffer, size_t bufferSize, uint32_t flags,
                        bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_set_port_owner(port_id port, team_id team);
BHAPI_IMPEXP status_t		_kern_write_port_etc(port_id port, int32_t msgCode,
                        const void *msgBuffer, size_t bufferSize, uint32_t flags,
                        bigtime_t timeout);
BHAPI_IMPEXP status_t		_kern_writev_port_etc(port_id id, int32_t msgCode,
...
BHAPI_IMPEXP status_t		_kern_set_debugger_breakpoint(void *address, uint32_t type,
...
BHAPI_IMPEXP void		_kern_atomic_set(int32_t *value, int32_t newValue);
BHAPI_IMPEXP int32_t		_kern_atomic_get_and_set(int32_t *value, int32_t newValue);
BHAPI_IMPEXP int32_t		_kern_atomic_test_and_set(int32_t *value, int32_t newValue,
                        int32_t testAgainst);
BHAPI_IMPEXP int32_t		_kern_atomic_add(int32_t *value, int32_t addValue);
BHAPI_IMPEXP int32_t		_kern_atomic_and(int32_t *value, int32_t andValue);
BHAPI_IMPEXP int32_t		_kern_atomic_or(int32_t *value, int32_t orValue);
BHAPI_IMPEXP int32_t		_kern_atomic_get(int32_t *value);
...
BHAPI_IMPEXP status_t		_kern_get_cpu_info(uint32_t firstCPU, uint32_t cpuCount,
                        cpu_info* info);
...
BHAPI_IMPEXP status_t		_kern_frame_buffer_update(addr_t baseAddress, int32_t width,
...
BHAPI_IMPEXP void			_kern_clear_caches(void *address, size_t length,
                        uint32_t flags);
BHAPI_IMPEXP bool			_kern_cpu_enabled(int32_t cpu);
BHAPI_IMPEXP status_t		_kern_set_cpu_enabled(int32_t cpu, bool enabled);

#if defined(__INTEL__) || defined(__x86_64__)
// our only x86 only syscall
BHAPI_IMPEXP status_t		_kern_get_cpuid(cpuid_info *info, uint32_t eax, uint32_t cpu);
#endif
...
BHAPI_IMPEXP partition_id	_kern_get_next_disk_device_id(int32_t *cookie,
                        size_t *neededSize);
...
BHAPI_IMPEXP status_t		_kern_get_next_disk_system_info(int32_t *cookie,
                        struct user_disk_system_info *info);
...
BHAPI_IMPEXP status_t		_kern_start_watching_disks(uint32_t eventMask, port_id port,
                        int32_t token);
BHAPI_IMPEXP status_t		_kern_stop_watching_disks(port_id port, int32_t token);
