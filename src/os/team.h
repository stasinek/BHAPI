...
status_t team_get_address_space(team_id id, struct VMAddressSpace **_addressSpace);
char **user_team_get_arguments(void);
int user_team_get_arg_count(void);
struct job_control_entry* team_get_death_entry(Team *team, thread_id child, bool* _deleteEntry);
void team_init_exit_info_on_error(Team* team);
bool team_is_valid(team_id id);
Team *team_get_team_struct_locked(team_id id);
int32_t team_max_teams(void);
int32_t team_used_teams(void);
thread_id load_image_etc(int32_t argCount, const char* const* args,const char* const* env, int32_t priority, team_id parentID, uint32_t flags);
void team_set_job_control_state(Team* team, job_control_state newState,Signal* signal);
void team_set_controlling_tty(int32_t index);
int32_t team_get_controlling_tty();
status_t team_set_foreground_process_group(int32_t ttyIndex, pid_t processGroup);
...
// used in syscalls.c
thread_id _user_load_image(const char* const* flatArgs, size_t flatArgsSize, int32_t argCount, int32_t envCount, int32_t priority, uint32_t flags, port_id errorPort, uint32_t errorToken);
...
pid_t _user_wait_for_child(thread_id child, uint32_t flags, siginfo_t* info);
status_t _user_exec(const char *path, const char* const* flatArgs,size_t flatArgsSize, int32_t argCount, int32_t envCount, mode_t umask);
thread_id _user_fork(void);
team_id _user_get_current_team(void);
pid_t _user_process_info(pid_t process, int32_t which);
...
status_t _user_get_next_team_info(int32_t *cookie, team_info *info);
status_t _user_get_team_usage_info(team_id team, int32_t who,	team_usage_info *info, size_t size);
status_t _user_get_extended_team_info(team_id teamID, uint32_t flags,	void* buffer, size_t size, size_t* _sizeNeeded);
