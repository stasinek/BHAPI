...
extern ssize_t		_kern_wait_for_objects(bhapi::object_wait_info* infos, int numInfos,
...
/* sem functions */
extern bhapi::sem_id		_kern_create_sem(int count, const char *name);
extern status_t		_kern_delete_sem(bhapi::sem_id id);
extern status_t		_kern_switch_sem(bhapi::sem_id releaseSem, bhapi::sem_id id);
extern status_t		_kern_switch_sem_etc(bhapi::sem_id releaseSem, bhapi::sem_id id,
                        uint32 count, uint32 flags, bigtime_t timeout);
extern status_t		_kern_acquire_sem(bhapi::sem_id id);
extern status_t		_kern_acquire_sem_etc(bhapi::sem_id id, uint32 count, uint32 flags,
                        bigtime_t timeout);
extern status_t		_kern_release_sem(bhapi::sem_id id);
extern status_t		_kern_release_sem_etc(bhapi::sem_id id, uint32 count, uint32 flags);
extern status_t		_kern_get_sem_count(bhapi::sem_id id, int32* thread_count);
extern status_t		_kern_get_sem_info(bhapi::sem_id semaphore, struct sem_info *info,
                        size_t size);
extern status_t		_kern_get_next_sem_info(bhapi::team_id team, int32 *cookie,
                        struct sem_info *info, size_t size);
extern status_t		_kern_set_sem_owner(bhapi::sem_id id, bhapi::team_id proc);
...
extern status_t		_kern_realtime_sem_close(bhapi::sem_id semID,
                        struct _sem_t** _deleteUserSem);
extern status_t		_kern_realtime_sem_unlink(const char* name);

extern status_t		_kern_realtime_sem_get_value(bhapi::sem_id semID, int* value);
extern status_t		_kern_realtime_sem_post(bhapi::sem_id semID);
extern status_t		_kern_realtime_sem_wait(bhapi::sem_id semID, bigtime_t timeout);

/* POSIX XSI semaphore syscalls */
extern int			_kern_xsi_semget(bhapi::key_t key, int numSems, int flags);
...
extern int			_kern_xsi_msgget(bhapi::key_t key, int messageQueueFlags);
...
extern bhapi::thread_id	_kern_load_image(const char* const* flatArgs,
                        size_t flatArgsSize, int32 argCount, int32 envCount,
                        int32 priority, uint32 flags, bhapi::port_id errorPort,
                        uint32 errorToken);
extern void __NO_RETURN _kern_exit_team(status_t returnValue);
extern status_t		_kern_kill_team(bhapi::team_id team);
extern bhapi::team_id		_kern_get_current_team();
extern status_t		_kern_wait_for_team(bhapi::team_id team, status_t *_returnCode);
extern pid_t		_kern_wait_for_child(bhapi::thread_id child, uint32 flags,
                        bhapi::siginfo_t* info);
extern status_t		_kern_exec(const char *path, const char* const* flatArgs,
                        size_t flatArgsSize, int32 argCount, int32 envCount,
                        mode_t umask);
extern bhapi::thread_id	_kern_fork(void);
...
extern bhapi::thread_id	_kern_spawn_thread(
                        struct thread_creation_attributes* attributes);
extern bhapi::thread_id	_kern_find_thread(const char *name);
extern status_t		_kern_suspend_thread(bhapi::thread_id thread);
extern status_t		_kern_resume_thread(bhapi::thread_id thread);
extern status_t		_kern_rename_thread(bhapi::thread_id thread, const char *newName);
extern status_t		_kern_set_thread_priority(bhapi::thread_id thread,
                        int32 newPriority);
extern status_t		_kern_kill_thread(bhapi::thread_id thread);
extern void			_kern_exit_thread(status_t returnValue);
extern status_t		_kern_cancel_thread(bhapi::thread_id threadID,
                        void (*cancelFunction)(int));
extern void			_kern_thread_yield(void);
extern status_t		_kern_wait_for_thread(bhapi::thread_id thread,
                        status_t *_returnCode);
extern bool			_kern_has_data(bhapi::thread_id thread);
extern status_t		_kern_send_data(bhapi::thread_id thread, int32 code,
                        const void *buffer, size_t bufferSize);
extern int32		_kern_receive_data(bhapi::thread_id *_sender, void *buffer,
                        size_t bufferSize);
extern int64		_kern_restore_signal_frame(
                        struct signal_frame_data* signalFrameData);

extern status_t		_kern_get_thread_info(bhapi::thread_id id, bhapi::thread_info *info);
extern status_t		_kern_get_next_thread_info(bhapi::team_id team, int32 *cookie,
                        bhapi::thread_info *info);
extern status_t		_kern_get_team_info(bhapi::team_id id, bhapi::team_info *info);
extern status_t		_kern_get_next_team_info(int32 *cookie, bhapi::team_info *info);
extern status_t		_kern_get_team_usage_info(bhapi::team_id team, int32 who,
                        team_usage_info *info, size_t size);
extern status_t		_kern_get_extended_bhapi::team_info(bhapi::team_id teamID, uint32 flags,
...
extern status_t		_kern_unblock_thread(bhapi::thread_id thread, status_t status);
extern status_t		_kern_unblock_threads(bhapi::thread_id* threads, uint32 count,
                        status_t status);

extern bigtime_t	_kern_estimate_max_scheduling_latency(bhapi::thread_id thread);
...
extern status_t		_kern_set_signal_mask(int how, const bhapi::sigset_t *set,
                        bhapi::sigset_t *oldSet);
extern status_t		_kern_sigaction(int sig, const struct sigaction *action,
                        struct sigaction *oldAction);
extern status_t		_kern_sigwait(const bhapi::sigset_t *set, bhapi::siginfo_t *info,
                        uint32 flags, bigtime_t timeout);
extern status_t		_kern_sigsuspend(const bhapi::sigset_t *mask);
extern status_t		_kern_sigpending(bhapi::sigset_t *set);
extern status_t		_kern_set_signal_stack(const stack_t *newStack,
                        stack_t *oldStack);

// image functions
extern bhapi::image_id		_kern_register_image(bhapi::image_info *info, size_t size);
extern status_t		_kern_unregister_image(bhapi::image_id id);
extern void			_kern_image_relocated(bhapi::image_id id);
extern void			_kern_loading_app_failed(status_t error);
extern status_t		_kern_get_bhapi::image_info(bhapi::image_id id, bhapi::image_info *info,
                        size_t size);
extern status_t		_kern_get_next_bhapi::image_info(bhapi::team_id team, int32 *cookie,
                        bhapi::image_info *info, size_t size);
extern status_t		_kern_read_kernel_image_symbols(bhapi::image_id id,
...
extern ssize_t		_kern_select(int numfds, struct fd_set *readSet,
                        ...
                        bigtime_t timeout, const bhapi::sigset_t *sigMask);
...
extern int			_kern_open_query(dev_t device, const char *query,
                        size_t queryLength, uint32 flags, bhapi::port_id port,
                        int32 token);
...
extern status_t		_kern_get_next_fd_info(bhapi::team_id team, uint32 *_cookie,
...
extern status_t		_kern_stop_notifying(bhapi::port_id port, uint32 token);
extern status_t		_kern_start_watching(dev_t device, ino_t node, uint32 flags,
                        bhapi::port_id port, uint32 token);
extern status_t		_kern_stop_watching(dev_t device, ino_t node, bhapi::port_id port,
                        uint32 token);
...
extern int32		_kern_create_timer(clockid_t clockID, bhapi::thread_id threadID,
                        uint32 flags, const struct sigevent* event,
                        const struct thread_creation_attributes*
                            threadAttributes);
extern status_t		_kern_delete_timer(int32 timerID, bhapi::thread_id threadID);
extern status_t		_kern_get_timer(int32 timerID, bhapi::thread_id threadID,
                        struct user_timer_info* info);
extern status_t		_kern_set_timer(int32 timerID, bhapi::thread_id threadID,
                        bigtime_t startTime, bigtime_t interval, uint32 flags,
                        struct user_timer_info* oldInfo);

// area functions
extern bhapi::area_id		_kern_create_area(const char *name, void **address,
                        uint32 addressSpec, size_t size, uint32 lock,
                        uint32 protection);
extern status_t		_kern_delete_area(bhapi::area_id area);
extern bhapi::area_id		_kern_area_for(void *address);
extern bhapi::area_id		_kern_find_area(const char *name);
extern status_t		_kern_get_area_info(bhapi::area_id area, area_info *info);
extern status_t		_kern_get_next_area_info(bhapi::team_id team, ssize_t *cookie,
                        area_info *info);
extern status_t		_kern_resize_area(bhapi::area_id area, size_t newSize);
extern bhapi::area_id		_kern_transfer_area(bhapi::area_id area, void **_address,
                        uint32 addressSpec, bhapi::team_id target);
extern status_t		_kern_set_area_protection(bhapi::area_id area,
                        uint32 newProtection);
extern bhapi::area_id		_kern_clone_area(const char *name, void **_address,
                        uint32 addressSpec, uint32 protection,
                        bhapi::area_id sourceArea);
...
extern bhapi::area_id		_kern_map_file(const char *name, void **address,
...
extern status_t		_kern_get_memory_properties(bhapi::team_id teamID,
                        const void *address, uint32* _protected, uint32* _lock);

/* kernel port functions */
extern bhapi::port_id		_kern_create_port(int32 queue_length, const char *name);
extern status_t		_kern_close_port(bhapi::port_id id);
extern status_t		_kern_delete_port(bhapi::port_id id);
extern bhapi::port_id		_kern_find_port(const char *port_name);
extern status_t		_kern_get_port_info(bhapi::port_id id, struct port_info *info);
extern status_t		_kern_get_next_port_info(bhapi::team_id team, int32 *cookie,
                        struct port_info *info);
extern ssize_t		_kern_port_buffer_size_etc(bhapi::port_id port, uint32 flags,
                        bigtime_t timeout);
extern int32		_kern_port_count(bhapi::port_id port);
extern ssize_t		_kern_read_port_etc(bhapi::port_id port, int32 *msgCode,
                        void *msgBuffer, size_t bufferSize, uint32 flags,
                        bigtime_t timeout);
extern status_t		_kern_set_port_owner(bhapi::port_id port, bhapi::team_id team);
extern status_t		_kern_write_port_etc(bhapi::port_id port, int32 msgCode,
                        const void *msgBuffer, size_t bufferSize, uint32 flags,
                        bigtime_t timeout);
extern status_t		_kern_writev_port_etc(bhapi::port_id id, int32 msgCode,
                        const struct iovec *msgVecs, size_t vecCount,
                        size_t bufferSize, uint32 flags, bigtime_t timeout);
extern status_t		_kern_get_port_message_info_etc(bhapi::port_id port,
                        port_message_info *info, size_t infoSize, uint32 flags,
                        bigtime_t timeout);

// debug support functions
extern status_t		_kern_kernel_debugger(const char *message);
extern void			_kern_register_syslog_daemon(bhapi::port_id port);
extern void			_kern_debugger(const char *message);
extern int			_kern_disable_debugger(int state);

extern status_t		_kern_install_default_debugger(bhapi::port_id debuggerPort);
extern bhapi::port_id		_kern_install_team_debugger(bhapi::team_id team,
                        bhapi::port_id debuggerPort);
extern status_t		_kern_remove_team_debugger(bhapi::team_id team);
extern status_t		_kern_debug_thread(bhapi::thread_id thread);
...
/* messaging service */
extern bhapi::area_id		_kern_register_messaging_service(bhapi::sem_id lockingSem,
                        bhapi::sem_id counterSem);
...
extern status_t		_kern_start_watching_disks(uint32 eventMask, bhapi::port_id port,
                        int32 token);
extern status_t		_kern_stop_watching_disks(bhapi::port_id port, int32 token);
