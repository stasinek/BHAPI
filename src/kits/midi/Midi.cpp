...
void BMidi::NoteOff(uchar channel, uchar note, uchar velocity, uint32_t time)
{
	// do nothing
}


void BMidi::NoteOn(uchar channel, uchar note, uchar velocity, uint32_t time)
{
	// do nothing
}
...
void BMidi::ProgramChange(uchar channel, uchar programNumber, uint32_t time)
{
	// do nothing
}


void BMidi::ChannelPressure(uchar channel, uchar pressure, uint32_t time)
{
	// do nothing
}


void BMidi::PitchBend(uchar channel, uchar lsb, uchar msb, uint32_t time)
{
	// do nothing
}


void BMidi::SystemExclusive(void* data, size_t length, uint32_t time)
{
	// do nothing
}


void BMidi::SystemCommon(uchar status, uchar data1, uchar data2, uint32_t time)
{
	// do nothing
}


void BMidi::SystemRealTime(uchar status, uint32_t time)
{
	// do nothing
}


void BMidi::TempoChange(int32_t beatsPerMinute, uint32_t time)
{
	// do nothing
}


void BMidi::AllNotesOff(bool justChannel, uint32_t time)
{
...
void BMidi::SnoozeUntil(uint32_t time) const
{
...
void BMidi::SpraySystemExclusive(
	void* data, size_t length, uint32_t time) const
{
...
void BMidi::SpraySystemRealTime(uchar status, uint32_t time) const
{
	fProducer->SpraySystemRealTime(status, MAKE_BIGTIME(time));
}


void BMidi::SprayTempoChange(int32_t beatsPerMinute, uint32_t time) const
{
