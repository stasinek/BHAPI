...
extern status_t add_node_listener(dev_t device, ino_t node, uint32_t flags,
	NotificationListener& listener);
...
extern status_t _user_stop_notifying(port_id port, uint32_t token);
extern status_t _user_start_watching(dev_t device, ino_t node, uint32_t flags,
