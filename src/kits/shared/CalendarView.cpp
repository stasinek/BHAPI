...
BCalendarView::BCalendarView(BRect frame, const char* name, uint32_t resizeMask,
	uint32_t flags)
	:
	...
	fStartOfWeek((int32_t)B_WEEKDAY_MONDAY),
...
BCalendarView::BCalendarView(const char* name, uint32_t flags)
	:
	BView(name, flags),
	BInvoker(),
	fSelectionMessage(NULL),
...
status_t BCalendarView::Archive(BMessage* archive, bool deep) const
{
...
void BCalendarView::AttachedToWindow()
{
...
void BCalendarView::FrameResized(float width, float height)
{
	Invalidate(Bounds());
}


void BCalendarView::Draw(BRect updateRect)
{
	if (LockLooper()) {
		if (fFocusChanged) {
			_DrawFocusRect();
...
void BCalendarView::DrawDay(BView* owner, BRect frame, const char* text,
...
void BCalendarView::DrawDayName(BView* owner, BRect frame, const char* text)
{
...
void BCalendarView::DrawWeekNumber(BView* owner, BRect frame, const char* text)
{
	// we get the full rect, fake this as the internal function
	// shrinks the frame to work properly when drawing a day item
	_DrawItem(owner, frame.InsetByCopy(-1.0, -1.0), text, true);
...
uint32_t BCalendarView::SelectionCommand() const
{
	if (SelectionMessage())
...
BMessage*  BCalendarView::SelectionMessage() const
{
	return fSelectionMessage;
}


void BCalendarView::SetSelectionMessage(BMessage* message)
{
...
uint32_t BCalendarView::InvocationCommand() const
{
	return BInvoker::Command();
}


BMessage*  BCalendarView::InvocationMessage() const
{
	return BInvoker::Message();
}


void BCalendarView::SetInvocationMessage(BMessage* message)
{
	BInvoker::SetMessage(message);
}


void BCalendarView::MakeFocus(bool state)
{
	if (IsFocus() == state)
		return;
...
status_t BCalendarView::Invoke(BMessage* message)
{
	bool notify = false;
	uint32_t kind = InvokeKind(&notify);
...
void BCalendarView::MouseDown(BPoint where)
{
	if (!IsFocus()) {
		MakeFocus();
		Sync();
...
void BCalendarView::KeyDown(const char* bytes, int32_t numBytes)
{
	const int32_t kRows = 6;
	const int32_t kColumns = 7;

	int32_t row = fFocusedDay.row;
	int32_t column = fFocusedDay.column;
...
void BCalendarView::ResizeToPreferred()
{
	float width;
	float height;
...
void BCalendarView::GetPreferredSize(float* width, float* height)
{
...
int32_t BCalendarView::Day() const
{
	return fDate.Day();
}


int32_t BCalendarView::Year() const
{
	int32_t year;
...
int32_t BCalendarView::Month() const
{
	int32_t month;
...
bool BCalendarView::SetDay(int32_t day)
{
	BDate date = Date();
	date.SetDay(day);
	if (!date.IsValid())
...
bool BCalendarView::SetMonth(int32_t month)
{
	if (month < 1 || month > 12)
		return false;
	BDate date = Date();
	int32_t oldDay = date.Day();
...
bool BCalendarView::SetYear(int32_t year)
{
	BDate date = Date();
...
bool BCalendarView::SetDate(const BDate& date)
{
	if (!date.IsValid())
		return false;
...
bool BCalendarView::SetDate(int32_t year, int32_t month, int32_t day)
{
...
void BCalendarView::SetStartOfWeek(BWeekday startOfWeek)
{
	if (fStartOfWeek == (int32_t)startOfWeek)
		return;

	fStartOfWeek = (int32_t)startOfWeek;
...
bool BCalendarView::IsDayNameHeaderVisible() const
{
	return fDayNameHeaderVisible;
}


void BCalendarView::SetDayNameHeaderVisible(bool visible)
{
...
bool BCalendarView::IsWeekNumberHeaderVisible() const
{
	return fWeekNumberHeaderVisible;
}


void BCalendarView::SetWeekNumberHeaderVisible(bool visible)
{
...
void BCalendarView::_InitObject()
{
...
void BCalendarView::_SetToDay()
{
	BDate date(fDate.Year(), fDate.Month(), 1);
	if (!date.IsValid())
		return;

	const int32_t firstDayOffset = (7 + date.DayOfWeek() - fStartOfWeek) % 7;

	int32_t day = 1 - firstDayOffset;
...
void BCalendarView::_GetYearMonthForSelection(const Selection& selection,
	int32_t* year, int32_t* month) const
{
	BDate startOfMonth(fDate.Year(), fDate.Month(), 1);
	const int32_t firstDayOffset
		= (7 + startOfMonth.DayOfWeek() - fStartOfWeek) % 7;
	const int32_t daysInMonth = startOfMonth.DaysInMonth();

	BDate date(fDate);
	const int32_t dayOffset = selection.row * 7 + selection.column;
...
void BCalendarView::_GetPreferredSize(float* _width, float* _height)
{
	BFont font;
	GetFont(&font);
...
void BCalendarView::_SetupDayNames()
{
...
void BCalendarView::_SetupDayNumbers()
{
	BDate startOfMonth(fDate.Year(), fDate.Month(), 1);
	...
	const int32_t lastDayBefore = lastDayInMonthBefore.DaysInMonth();

	int32_t counter = 0;
	int32_t firstDayAfter = 1;
	for (int32_t row = 0; row < 6; ++row) {
		for (int32_t column = 0; column < 7; ++column) {
			int32_t day = 1 + counter - firstDayOffset;
...
void BCalendarView::_SetupWeekNumbers()
{
	BDate date(fDate.Year(), fDate.Month(), 1);
	if (!date.IsValid())
		return;
...
void BCalendarView::_DrawDay(int32_t currRow, int32_t currColumn, int32_t row,
	int32_t column, int32_t counter, BRect frame, const char* text, bool focus)
{
	BDate startOfMonth(fDate.Year(), fDate.Month(), 1);
	const int32_t firstDayOffset
		= (7 + startOfMonth.DayOfWeek() - fStartOfWeek) % 7;
	const int32_t daysMonth = startOfMonth.DaysInMonth();
...
void BCalendarView::_DrawDays()
{
	BRect frame = _FirstCalendarItemFrame();

	const int32_t currRow = fSelectedDay.row;
	const int32_t currColumn = fSelectedDay.column;

	const bool isFocus = IsFocus();
	const int32_t focusRow = fFocusedDay.row;
	const int32_t focusColumn = fFocusedDay.column;
...
void BCalendarView::_DrawFocusRect()
{
	BRect frame = _FirstCalendarItemFrame();
...
void BCalendarView::_DrawDayHeader()
{
	if (!fDayNameHeaderVisible)
		return;
...
void BCalendarView::_DrawWeekHeader()
{
	if (!fWeekNumberHeaderVisible)
		return;
...
void BCalendarView::_DrawItem(BView* owner, BRect frame, const char* text,
	bool isSelected, bool isEnabled, bool focus)
{
...
void BCalendarView::_UpdateSelection()
{
	BRect frame = _FirstCalendarItemFrame();
		...
		for (int32_t column = 0; column < 7; ++column) {
			...
			if (tmp.Contains(where)) {
				...
				if (month == fDate.Month()) {
					...
					// selected
					int32_t day = atoi(fDayNumbers[row][column].String());
